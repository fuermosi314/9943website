...
export default function ConsumablesPage() {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a1a]">
      {/* 顶部导航 */}
      <div className="fixed top-0 left-0 right-0 z-40 bg-[#0a0a1a]/80 backdrop-blur-md border-b border-white/10 safe-area-top">
        ...
        <div className="glass-card rounded-xl p-3 mb-4">
          ...
          <div className="flex flex-wrap items-center gap-2 mt-3">
            <button
              ...
              className={`px-3 py-2 rounded-full text-sm transition-colors ${filterCat === 'all' ? 'bg-[#fb6400] text-white' : 'bg-white/10 text-white/60 hover:bg-white/20'}`}
            >
              全部
            </button>
            {customCategories.map(cat => (
              <button
                ...
                className={`px-3 py-2 rounded-full text-sm transition-colors ${filterCat === cat.name ? 'bg-[#fb6400] text-white' : 'bg-white/10 text-white/60 hover:bg-white/20'}`}
              >
                {cat.icon} {cat.name}
              </button>
            ))}
          ...
          <div className="flex items-center gap-2 mt-3 text-sm text-white/50">
            ...
            {(['name', 'quantity', 'price', 'storageDate'] as SortField[]).map(f => (
              <button
                ...
                className={`px-3 py-2 rounded text-sm ${sortField === f ? 'bg-white/20 text-white' : 'hover:bg-white/10'}`}
              >
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/60" onClick={onClose}>
      ...
      >
        <div className="p-4 border-b border-white/10 flex items-center justify-between">
          ...
          <button onClick={onClose} className="w-10 h-10 flex items-center justify-center text-white/50 hover:text-white text-xl">×</button>
        </div>
