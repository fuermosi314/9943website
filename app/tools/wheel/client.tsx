                ...
                {showPresetInput ? (
                  <div className="flex gap-2">
                    <input
                      ...
                      placeholder="类别名称"
                      className="flex-1 px-3 py-2 bg-white/5 border border-white/10 rounded-lg text-white text-sm focus:outline-none focus:border-[#fb6400]"
                      autoFocus
                      onKeyDown={e => { if (e.key === 'Enter' && presetName.trim()) { saveAsCustomPreset(presetName.trim()); setPresetName(''); setShowPresetInput(false); } }}
                    />
                    <button
                      onClick={() => { if (presetName.trim()) { saveAsCustomPreset(presetName.trim()); setPresetName(''); setShowPresetInput(false); } }}
                      className="px-4 py-2 bg-[#fb6400] hover:bg-[#e55a00] text-white rounded-lg text-sm font-medium transition-colors"
                    >保存</button>
                    <button
                      onClick={() => { setShowPresetInput(false); setPresetName(''); }}
                      className="px-3 py-2 bg-white/5 hover:bg-white/10 text-white/50 rounded-lg text-sm transition-colors"
                    >取消</button>
                  </div>
                ) : (
                  <button
