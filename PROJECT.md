...
- [x] **【最高优先级】网站智能客服（AI 问答助手）** —— **已完成并端到端实测**
  ...
  - **部署**：Vercel，零额外服务器；沿用现有 `DEEPSEEK_*` 环境变量
    - **⚠️ 函数区域不要改成 `hkg1`——试过，会让客服直接挂掉，已回滚到默认的 `iad1`**。
      （2026-09-16 实测记录）改到香港后，`fetch` 调百炼开始**间歇性失败**，
      日志报 `[chat] 上游调用失败 fetch failed`，接口对外返回 502（Cloudflare 的
      `error code: 502`）。改回 iad1 后恢复。**同样是慢，但 iad1 是能成功的**
    - **真正的瓶颈：Vercel 在境外，百炼在国内。** 实测（同一 Key、同一模型、同一提示词）：
      从国内直连百炼 TTFB **1.1~1.3s**，从 Vercel（iad1/hkg1 都一样）**~11s**，
      差 10 倍。且百炼从国内一直健康（0.83~0.99s），所以不是服务商的问题，是跨境链路
    - 已排除的怀疑（都实测过，别再往这些方向查）：Redis 只占 **0.36s**；
      函数启动约 1.5s；`enable_thinking: false` 确实生效（本地因此从 6.97s → 1.2s）
    - **想根治只有换接入点**：百炼有国际版 `dashscope-intl.aliyuncs.com`（新加坡节点，
      从境外可达），但**国内站的 Key 在它上面无效**（实测报 `invalid_api_key`，
      报错指向 alibabacloud.com 而非 aliyun.com，属两套账号体系），需要另开国际站账号
    - 判断函数实际落在哪个区：看响应头的 `x-vercel-id`，**第二段**就是函数区
      （如 `sfo1::iad1::xxx` = 边缘 sfo1、函数 iad1）
    - **验证线上是否真的流式**：`curl -sN` 打 `/api/chat` 看分块是否逐条到达。
      本站前面有 Cloudflare，它对 `text/event-stream` 不缓冲（已实测确认）
