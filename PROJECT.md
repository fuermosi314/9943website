...
- [x] **【最高优先级】网站智能客服（AI 问答助手）** —— **已完成并端到端实测**
  ...
  - **部署**：Vercel，零额外服务器；沿用现有 `DEEPSEEK_*` 环境变量
    - **函数区域必须是亚洲（`vercel.json` 的 `"regions": ["hkg1"]`）**。
      原先用默认的 `iad1`（美东），而**模型 API 阿里云百炼在国内**
      （从国内连它：连接 0.026s、TLS 0.070s），于是每次客服提问都要跨太平洋跑一个来回。
      实测首字 8.27 / 14.10 / 9.16s，同一问题本地直连只要 2.21s。
      改到 `hkg1`（香港）后，访客→函数、函数→模型两段跨洋链路一起消失。
      **改区域前先确认 Upstash 在哪个区**——它若在美东，Redis 两次往返会从 ~10ms 变成 ~200ms
      （+0.4s），但仍远小于跨太平洋省下的时间
