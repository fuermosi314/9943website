  ...
  if (!isOpen) {
    return (
      <button
        ...
        // 让开右下角的全局客服气泡。必须用 calc(6rem + safe-area) 而不是 bottom-24：
        // 客服气泡带 env(safe-area-inset-bottom)，在 iPhone 上会上移约 34px，
        // 固定 96px 会被它顶到，两个圆按钮重叠约 18px
        className="fixed right-6 w-14 h-14 bg-gradient-to-br from-purple-600 to-blue-600 rounded-full flex items-center justify-center shadow-2xl shadow-purple-500/30 hover:shadow-purple-500/50 transition-all hover:scale-105 z-50"
